...
class RapportController extends Controller
{
    ...
    public function generateRapportForm()
    {
        ...
        $listeventesacademic = TFacture::where('numvente', 'like', 'POS%')
    ...
    public function generateRapport(Request $request)
    {
        ...
        $query = TFacture::query()
            ->where('numvente', 'like', 'POS%'); // Filtrer uniquement les ventes qui commencent par "POS"
        ...
        if ($timeframe === 'day' && $startDate && $endDate) {
            ...
            // Si aucun critère de période n'est défini, appliquer la plage de dates directement
        ...
        // Remplissage des données dynamiques dans le tableau
    ...
    public function generateRapportByMontly(Request $request)
    {
        // Récupérer les critères envoyés par Alpine.js
        $timeframe = $request->input('timeframe'); // 'day', 'month', 'week'
        $startDate = $request->input('start_date'); // Date début
        ...
        // Construire la requête de base
        $query = TFacture::query()
            ->where('numvente', 'like', 'POS%'); // Filtrer uniquement les ventes qui commencent par "vp"
