        ...
        <div class="app-container container-xxl">
            <div class="card ">
                                                ...
                                                <select class="form-select" x-model="emploi.matiere_id" required>
                                                    ...
                                                    <template x-for="matiere in matieres" :key="matiere.id">
                                                        <option :value="matiere.id" x-text="matiere.name"
                                                            :selected="emploi.matiere_id == matiere.id"></option>
                                                    </template>
                                                </select>
                                            </td>
                                            ...
                                            <td>
                                                <select class="form-select" x-model="emploi.jour_id" required>
                                                    ...
                                                    <template x-for="jour in jours" :key="jour.id">
                                                        <option :value="jour.id" x-text="jour.name"
                                                            :selected="emploi.jour_id == jour.id"></option>
                                                    </template>
                                                </select>
                                            </td>
                ...
                <div class="card-footer d-flex justify-content-between">
                    ...
                    <button class="btn btn-info btn-sm" @click="printEmploiDuTemps()">
        ...
        function emploiDuTemps() {
            return {
                ...
                submitEmploiDuTemps() {
                    ...
                    const champsInvalides = this.emplois.some(e =>
                        !e.matiere_id || !e.jour_id || !e.heure_debut || !e.heure_fin
                    );
                    ...
                    if (champsInvalides) {
                        Swal.fire({
                            ...
                            text: 'Veuillez remplir toutes les lignes de l’emploi du temps.',
                        });
                        return;
                    }
                ...
                printEmploiDuTemps() {
                    fetch('{{ route('administration.configuration.print.emploi.temps') }}', {
                        ...
                        .then(data => {
                            window.open(data.url, '_blank'); // ✅ ouvrir le PDF généré
                        })
                        .catch(error => {
                            Swal.fire({
                                icon: 'error',
